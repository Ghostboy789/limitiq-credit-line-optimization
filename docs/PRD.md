...
Internal APIs retain the field name `pd`, but v4 product copy calls it a
...
6. Governance leads with behavioral-primary test evidence and separately shows
...
- Mixed-integer allocation selects exactly one action per account within
  portfolio growth, loss-growth, capital and higher-risk concentration caps;
  per-account proposals remain within maximum exposure.
