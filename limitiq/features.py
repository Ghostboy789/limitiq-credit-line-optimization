...
MODEL_INPUT_COLUMNS = ["LIMIT_BAL", *PAY_COLUMNS, *BILL_COLUMNS, *PAYMENT_COLUMNS]
BATCH_COLUMNS = ["ACCOUNT_ID", *MODEL_INPUT_COLUMNS]
FEATURE_NAMES = [
    "limit_bal",
    "current_utilization",
    "average_utilization",
...
def validate_input(frame: pd.DataFrame, *, require_account_id: bool = False) -> pd.DataFrame:
    required = BATCH_COLUMNS if require_account_id else MODEL_INPUT_COLUMNS
    ...
    numeric = MODEL_INPUT_COLUMNS
    converted = clean[numeric].apply(pd.to_numeric, errors="coerce")
    invalid = converted.isna() & clean[numeric].notna()
    if invalid.any().any() or converted.isna().any().any():
        bad = sorted(
            set(invalid.columns[invalid.any()]) | set(converted.columns[converted.isna().any()])
        )
    ...
    if (clean["LIMIT_BAL"] > 10_000_000).any():
        raise SchemaError("LIMIT_BAL exceeds the educational demo maximum of 10,000,000")
...
def clean_source(frame: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    required = [ID_COLUMN, *MODEL_INPUT_COLUMNS, *DEMOGRAPHIC_COLUMNS, TARGET]
