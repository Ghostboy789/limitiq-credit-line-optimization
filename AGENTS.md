...
Python 3.11 FastAPI/Jinja credit-risk research application with Ruflo coordination metadata.
...
### Test
```powershell
.\.venv\Scripts\python.exe -m pytest --cov=limitiq --cov-report=term-missing
ruff check .
ruff format --check .
```

Read `HANDOVER.md` before changing v2. Do not redownload `data/raw`, and do not
commit, push, tag or deploy v2 while the publication gate is blocked.
...
- Preserve observed/model-estimated/simulated evidence boundaries

### File Organization
- `/limitiq` - Application, modelling, optimization and reporting code
- `/tests` - Test files
- `/docs` - Documentation
- `/models` - Versioned model artifacts
- `/reports` - Generated evidence and executive reports
- `/data/raw` - Gitignored source files; never publish
